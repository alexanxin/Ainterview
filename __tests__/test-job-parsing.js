...
// Copy the same parsing logic from the updated route
async function testJobParsing(htmlContent) {
    ...
    $("script, style, nav, header, footer, aside, menu, link, meta, noscript").remove();
    ...
    if (foundContent && (mainContent.toLowerCase().includes('footer') ||
        ...
        console.log("Detected footer or navigation content, trying more specific selectors");
        foundContent = false;
        mainContent = "";
    }
    ...
    if (textContent.length > 10 && hasJobPhrases) {
        console.log("Content would be accepted - has job phrases and sufficient length");
        return {
            success: true,
        ...
        return {
            success: false,
            message: "Could not extract meaningful job posting content from the provided HTML",
            contentLength: textContent.length,
            hasJobPhrases: hasJobPhrases
        };
    }
...
function extractDescriptionFromJson(data) {
    if (!data) return null;
    ...
    try {
        ...
        if (data.props?.pageProps?.network?.jobs?.found) {
            ...
            if (Array.isArray(jobsArray)) {
                const job = jobsArray.find((j) => j.description);
                if (job) {
...
async function testSpecificJobUrl() {
    const url = 'https://jobs.solana.com/companies/monkedao-2/jobs/60969275-product-manager-banana-stand-platform';
