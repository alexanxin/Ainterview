...
// Corrected test script to check PYUSD balance on connected wallet
// This script handles both SPL Token and attempts to detect Token-2022
async function checkCorrectedPYUSDBalance(walletAddress: string) {
  console.log("🔍 Corrected PYUSD balance checker");
  ...
  try {
    ...
    console.log(`Token-2022 Program ID: ${tokenProgram202.toString()}`);
    ...
    // Get Token-2022 accounts - try to detect them by looking at account owners
    // Since we don't have spl-token-202 package, we'll try to identify them by looking at account owners
    try {
      ...
      const token202Accounts = await connection.getTokenAccountsByOwner(
        walletPublicKey,
      ...
      console.log(
        "⚠️  Could not fetch Token-202 accounts directly:",
        token2022Error
      );
    }
    ...
    for (const tokenAccount of allTokenAccounts) {
      try {
        ...
        if (accountBalance.value.mint === pyusdMint) {
          ...
          if (accountInfo?.owner.toString() === tokenProgram202.toString()) {
            console.log(
...
console.log("🚀 Starting Corrected PYUSD Balance Check Test\n");
checkCorrectedPYUSDBalance(testWallet)
  .then((balance) => {
...
export { checkCorrectedPYUSDBalance, createPYUSDTransferTransaction };
